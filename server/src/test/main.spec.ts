...
import "jest";
import {DatabaseConnector, GRAPHIQL_ROUTE, GRAPHQL_ROUTE, main} from "./main";
...
describe("main", () => {
  it("should be able to Initialize a server (production)", () => {
    return main({
      database: "mongodb://localhost/simeat",
      enableCors: false,
...
describe("TestConnector", () => {
  it("should pass sanity", () => {
    expect(typeof DatabaseConnector).toBe("function");
  });

  it("return string", () => {
    const tc = new DatabaseConnector();
    expect(tc.connect("DatabaseConnector")).toBe("it works from connector as well!");
  });
});
